...
function Flashcards() {
  ...
  const fetchFlashcards = (callback) => {
    ...
    api.get(`/api/note/`)
      .then((res) => {
        ...
        console.log("Fetched Flashcard Data:", data); // DEBUG LOG 1
        setFlashcards(data);
 ...
 useEffect(() => {
    ...
    console.log("Setting Displayed Flashcards:", ordered); // DEBUG LOG 2
  ...
  const handleCreateNew = () => {
   ...
   const handleEditClick = (card) => {
       console.log("Editing card:", card); // DEBUG LOG 3
   ...
   const handleSaveFlashcard = (e) => {
    ...
    const handleDeleteClick = (cardId) => {
        console.log("handleDeleteClick called with ID:", cardId); // DEBUG LOG 4
        if (!cardId) {
            console.error("Delete failed: Card ID is missing or invalid.");
        ...
        {!loading && displayedFlashcards.length > 0 ? (
            <table className="flashcard-list-table">
                ...
                {displayedFlashcards.map((card) => {
                    console.log("Rendering row for card:", card); // DEBUG LOG 5
                    return (
                        <tr key={card.id}>
