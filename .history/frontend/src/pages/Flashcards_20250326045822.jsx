...
function Flashcards() {
 ...
 useEffect(() => {
    ...
    if (cardOrder === "Random" && activeMode === 'study') { // Only shuffle in study mode? Or always?
  ...
  }, [flashcards, selectedModule, cardOrder, activeMode]); // Added activeMode
  ...
  const handleCreateNew = () => {
   ...
   const handleSaveFlashcard = (e) => {
    ...
    const handleDeleteClick = (cardId) => {
        if (!cardId) {
            console.error("Delete failed: cardId is missing.");
        ...
        if (window.confirm("Are you sure you want to delete this flashcard?")) {
            ...
            api.delete(`/api/note/${cardId}/`)
                .then(() => {
                    ...
                    setManageViewMode("list"); // Stay in list view
                    setFlashcards(prev => prev.filter(card => card.id !== cardId)); // Optimistic update
                })
                .catch(err => {
                    ...
                    fetchFlashcards(); // Refetch on error
                })
                .finally(() => {
                    setLoading(false);
                });
    ...
    const renderStudyMode = () => (
        <div className="flashcard-study-mode">
            ...
            <div className="flashcard-view-controls">
                ...
                <button className={`fc-button fc-button-info ${studyView === 'title+back' ? 'active' : ''}`} style={{backgroundColor: '#a0d2eb', color:'#4a7c94'}} onClick={() => setStudyView("title+back")} disabled={totalCards === 0} > View Both </button>
            </div>
        </div>
    );
