...
function Flashcards() {
  ...
  const [activeMode, setActiveMode] = useState("manage");
 ...
 useEffect(() => {
    ...
    if (cardOrder === "Random" && activeMode === 'study') {
  ...
  }, [flashcards, selectedModule, cardOrder, activeMode]);
  ...
  const handleCreateNew = () => {
   ...
   const handleSaveFlashcard = (e) => {
    ...
    const handleDeleteClick = (cardId) => {
        if (!cardId) {
            console.error("Delete failed: cardId is undefined or null.");
  ...
  const handleMarkHard = () => { const card = displayedFlashcards[currentCardIndex]; if (!card) return; alert("Mark as Hard - Not implemented."); };
  const handleMarkEasy = () => { const card = displayedFlashcards[currentCardIndex]; if (!card) return; alert("Mark as Easy - Not implemented."); };
  ...
  const handleResetProgress = () => { setIsCardAnimating(true); setTimeout(() => { setCurrentCardIndex(0); setQuestionsAnswered(0); if (cardOrder === "Random") { setDisplayedFlashcards(shuffleArray([...displayedFlashcards])); } setStudyView('title'); setIsCardAnimating(false); alert("Progress Reset."); }, 300); };
  ...
  const renderCurrentCardContent = () => {
   ...
   const renderListView = () => (
    <div className="flashcard-list-view">
        ...
        {loading && manageViewMode === 'list' && <p className="loading-indicator">Loading...</p>}
        ...
        {!loading && displayedFlashcards.length > 0 ? (
            <table className="flashcard-list-table">
                ...
                {displayedFlashcards.map((card) => (
                    <tr key={card?.id || Math.random()}>
                        <td>{card?.title || 'N/A'}</td><td>{card?.type || 'general'}</td>
    ...
    const renderStudyMode = () => (
        <div className="flashcard-study-mode">
             <div className="flashcard-top-controls">
                ...
                <button className="fc-button fc-button-placeholder" onClick={handlePrevious} disabled={totalCards === 0 || isCardAnimating}>Previous</button>
                <button className="fc-button fc-button-placeholder" onClick={handleNext} disabled={totalCards === 0 || isCardAnimating}>Next</button>
                <button className="fc-button fc-button-placeholder" onClick={handleMarkHard} disabled={totalCards === 0}>Mark Hard</button>
                <button className="fc-button fc-button-placeholder" onClick={handleMarkEasy} disabled={totalCards === 0}>Mark Easy</button>
                <button className="fc-button fc-button-placeholder" onClick={handleReviewAll}>Review All</button>
                <button className="fc-button fc-button-placeholder" onClick={handleResetProgress}>Reset</button>
            </div>
            ...
            <div className="flashcard-stats-bar">
             ...
             {loading && activeMode === 'study' && <p className="loading-indicator">Loading...</p>}
             {error && !loading && activeMode === 'study' && <p className="error-message">{error}</p>}
            ...
            <div className="flashcard-view-controls">
                <button className={`fc-button fc-button-view-title ${studyView === 'title' ? 'active' : ''}`} onClick={() => setStudyView("title")} disabled={totalCards === 0} > View Title </button>
                <button className={`fc-button fc-button-view-back ${studyView === 'back' ? 'active' : ''}`} onClick={() => setStudyView("back")} disabled={totalCards === 0} > View Back </button>
                <button className={`fc-button fc-button-view-both ${studyView === 'title+back' ? 'active' : ''}`} onClick={() => setStudyView("title+back")} disabled={totalCards === 0} > View Both </button>
            </div>
        </div>
    );
