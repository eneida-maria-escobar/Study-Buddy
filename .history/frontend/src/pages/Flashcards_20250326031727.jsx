...
function Flashcards() {
  ...
  const handlePrevious = () => {
    setCurrentCardIndex(prev => (prev > 0 ? prev - 1 : displayedFlashcards.length - 1));
  };

  const handleNext = () => {
    setCurrentCardIndex(prev => (prev < displayedFlashcards.length - 1 ? prev + 1 : 0));
    setQuestionsAnswered(prev => prev + 1);
  };

  const handleMarkHard = () => {
    console.log("Marking card as hard (not implemented):", displayedFlashcards[currentCardIndex]?.id);
    alert("Mark as Hard - Not fully implemented.");
  };

  const handleMarkEasy = () => {
    console.log("Marking card as easy (not implemented):", displayedFlashcards[currentCardIndex]?.id);
  ...
  return (
    <div className="flashcards-main-card">
      ...
      <div className="flashcard-top-controls">
         ...
         <button className="btn-functional btn-prev-next" onClick={handlePrevious} disabled={totalCards === 0}>Previous</button>
         <button className="btn-functional btn-prev-next" onClick={handleNext} disabled={totalCards === 0}>Next</button>
      ...
      <div className="flashcard-view-controls">
        <button
          className={`btn btn-view-title ${view === 'title' ? 'active' : ''}`}
        ...
        <button
          className={`btn btn-view-back ${view === 'back' ? 'active' : ''}`}
        ...
        <button
          className={`btn btn-view-both ${view === 'title+back' ? 'active' : ''}`}
      ...
      <div className="create-flashcard-form-area">
        ...
        <form onSubmit={handleCreateFlashcard}>
          ...
          <div className="form-group">
            ...
            <input type="text" className="form-control" id="typeInput" name="type" value={newFlashcard.type} onChange={handleInputChange} placeholder="e.g., Vocabulary (defaults to 'general')" />
          </div>
